function vtimeweigh=vint2timeweighed(vint,t,tout)
...
if(nargin < 3)
	...
	nt=length(t);
	i1=1:nt-1;
	vrms(i1)=cumsum(dt.*(vint(i1).^n));

	vrms(i1)=(vrms(i1)./(t(i1+1)-t(1)));
	vrms(end)=vrms(end-1);
else
   ind=find(tout<t(1));
   if(~isempty(ind))
